...
def evaluate_metric(model, data_iter, scaler):
    ...
    with torch.no_grad():
        ...
        for x, y in data_iter:
            ...
            # plt.plot(mape)
            # plt.show()
            mask = np.where(y>=1E-3) # 我的代码,防止分母过小
            mask_mae += (np.array(d/y)[mask]).tolist()
        # plt.plot(mask_mae)    
        MAE = np.array(mae).mean()
        MAPE = np.array(mape).mean()
        RMSE = np.sqrt(np.array(mse).mean())
        WMAPE = np.sum(np.array(mae)) / np.sum(np.array(sum_y))
        MASK_MAPE = np.array(mask_mae).mean()

        #return MAE, MAPE, RMSE
        return MAE, RMSE, WMAPE,MAPE,MASK_MAPE
...
def show_curve(model, data_iter, scaler,node,args,enable_gap):
    model.eval()
    s = 0
    with torch.no_grad():
        for x, y in data_iter:
            s+=1
            # y_pred = model(x).squeeze().cpu().numpy()[:,node]
            # y = y.squeeze().cpu().numpy()[:,node]
            # plt.figure()
            # plt.plot(y,label = 'ground_truth')
            # plt.plot(y_pred,label = 'predicted_value')
            # plt.show()
            # h = x = np.arange(0,len(x)*5,5)
            y = scaler.inverse_transform(y.cpu().numpy())[:,node]
            y_pred = scaler.inverse_transform(model(x).view(len(x), -1).cpu().numpy())[:,node]

            plt.figure()
            plt.plot(y,label = 'ground_truth')
            plt.plot(y_pred,label = 'predicted_value')
            if enable_gap:
               plt.plot(y-y_pred,label = 'gap between truth and pred_value')
            else:
                pass
            plt.legend(loc = "upper right",bbox_to_anchor=(0.9,1.1),
           ncol = 3,title = "traffic prediction",shadow=False,fancybox=True)
            plt.legend(loc = "upper right",bbox_to_anchor=(0.7,1.1),
           ncol = 3,title = f"Dataset:{args.dataset} |Time_step:{args.n_pred*args.time_intvl}Min |Node:#{node}",shadow=False,fancybox=True)
            plt.show()
            # plt.hist(y-y_pred)
            if s>=4:
                print('太多了不画了，只画前四个')
