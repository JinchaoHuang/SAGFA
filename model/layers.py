...
class Align(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
# f = torch.randn(13,1,12,207)
# N = Align(c_in=1, c_out=8)
# N(f).size() #13-8-12-207   
...
class Align3d(nn.Module):
    ...
    def forward(self, x):
        if self.c_in > self.c_out:
            ...
            x = x
        return x
# f = torch.randn(13,12,207)
# N = Align3d(c_in=12, c_out=512)
# N(f).size() #torch.Size([13, 512, 207])  




# 输入张量进行1D卷积，输入shape为BCX，最后一个维度进行卷积操作，长度会缩短(k-1),用不到忽略
...
# 对四维张量最后两个维度同时进行2d卷积，会是的两个维度长度都缩短kernel-1,
# 如果enable_padding=True，则只有倒数第二维度缩短kernel-1
# B C T S to B C' T' S', T' = T-(k-1), S' = S-(k-1)
class CausalConv2d(nn.Conv2d):
    ...
    def forward(self, input):
        ...
        return result
# f = torch.randn(13,63,12,207)
# N = CausalConv2d(in_channels=63, out_channels=17, kernel_size=5,enable_padding=True)
# N(f).size() #13-17-8-207

# 标准的GLU单元，B C T S -> B C T' S, T' = T-kt+1
...
# f = torch.randn(13,63,12,207)
# N = TemporalConvLayer(Kt=5,c_in=63,c_out=17,n_vertex=229,act_func='glu')
# N(f).size() #13-17-8-207


# GCN 单元，B C T S--> B T S C'
...
# f = torch.randn(13,63,12,229)
# g = torch.randn(229,229)
# N = ChebGraphConv(c_in=63, c_out=17, Ks=4, gso=g, bias=None)
# N(f).size() # 13-12-229-17  
...
# f = torch.randn(13,63,12,229)
# g = torch.randn(229,229)
# N = GraphConv(c_in=63, c_out=17, gso=g, bias=None)
# N(f).size() # 13-12-229-17  

# 先通道对齐，再GCN,然后与GCN输入相加（残差连接）
# B C T S --> B C' T S, 即C_in to C_out
...
# f = torch.randn(13,63,12,229)
# g = torch.randn(229,229)
# N = GraphConvLayer(graph_conv_type='graph_conv',c_in=63, c_out=17,Ks=3, gso=g, bias=None)
# N(f).size() # 13-17-12-229      
...
# f = torch.randn(13,63,12,207)
# g = torch.randn(207,207)
# N = STConvBlock(Kt=3,Ks=3,n_vertex=207,last_block_channel=63,channels=[31,63,33],
#                 act_func='glu',graph_conv_type='cheb_graph_conv',
#                 gso=g,bias=None,droprate=0.5)
# N(f).size() # 13-33-8-207
...
class OutputBlock(nn.Module):
    ...
    def __init__(self, Ko, last_block_channel, channels, end_channel, n_vertex, act_func, bias, droprate):
        ...
        # kernel size, c_in, c_out, n_vertex, act_func
    ...
    def forward(self, x):
        x = self.tmp_conv1(x) # 卷积 是的时间缩减为1： B C T=1 S
        x = self.tc1_ln(x.permute(0, 2, 3, 1)) # layer norm B T=1 S C ==》layer norm
        x = self.fc1(x)   # 全连接 B T=1 S C
        x = self.relu(x)  # 激活
        x = self.dropout(x)  # dropout
        x = self.fc2(x).permute(0, 3, 1, 2) # 全连接 B C T S
        return x
f = torch.randn(13,8,12,207)
N = OutputBlock(12, 8, [32,64], 1, 207, 'glu', True, 0.3)
# # (输入步长ko，输入通道数，全连接通道数*2，输出通道数，顶点数，TCN激活函数，全连接是否有bias，droprate)
N(f).size() #13-32-9-207
