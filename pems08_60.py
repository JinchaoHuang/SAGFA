...
def get_parameters():
    ...
    parser.add_argument('--n_heads', type=int, default=8, help='num of heads') # 8 for pems08_60mins only
    parser.add_argument('--emb_dim', type=int, default=8, help='node embedding dimension') 
    parser.add_argument('--fb_dim', type=int, default=8, help='fblock dimension') 
    
    parser.add_argument('--batch_size', type=int, default=32)
    parser.add_argument('--epochs', type=int, default=500, help='epochs, default as 10000')
    ...
    parser.add_argument('--save_path', type=str, default='./ablation_exp/temp_pems07_60/sago/', help='save path')
    parser.add_argument('--dataset', type=str, default='pems07', choices=['metr-la', 'pems-bay', 'pemsd7-m','pems03','pems04','pems08'])
...
class Configs:
    def __init__(self,args,n_vertex):
        self.seq_len = args.n_his*1*args.n_heads # args.n_heads  # 输入序列长度12*8
        self.pred_len = 1  # 预测序列长度3
        self.d_model = args.fb_dim  # 模型的维度512
        self.factor = 3  # 用于缩放注意力机制的因子5
        self.n_heads = 8  # 注意力头的数量8
        self.e_layers = 3  # 编码器的层数3
        self.d_ff = args.fb_dim  # 前馈神经网络的维度2048
        ...
        self.fnet_d_ff = args.fb_dim # 频域前馈神经网络的维度1024
        self.fnet_d_model = args.fb_dim  # 频域模型的维度512
...
class Configs1:
    def __init__(self,args,n_vertex):
        ...
        self.pred_len = args.n_his*args.n_heads  # 预测序列长度3
        self.d_model = args.fb_dim  # 模型的维度512
        self.factor = 3  # 用于缩放注意力机制的因子5
        self.n_heads = 8  # 注意力头的数量8
        self.e_layers = 3  # 编码器的层数3
...
def prepare_model(args, n_vertex):
    ...
    if args.model == 'sagbf':
        # model = models.FB(args, n_vertex, configs).to(device) #注意此处要设置device！！！
        # model = models.SFB(args, n_vertex, configs).to(device) #注意此处要设置device！！！
        model = models.SAGO(args, n_vertex, configs).to(device) #注意此处要设置device！！！
        # model = models.AGFB(args, n_vertex, configs).to(device) #注意此处要设置device！！！
        # model = models.SAGFB(args, n_vertex, configs).to(device) #注意此处要设置device！！！
        # model = models.SAGFBO(args, n_vertex, configs1).to(device) #注意此处要设置device！！！
    if args.opt == "rmsprop":
...
def train(loss, args, optimizer, scheduler, es, model, train_iter, val_iter):
    min_loss = 10
    # continue train from saved checkpoint
    # model.load_state_dict(torch.load(args.save_path+'195.pth'))
    # args.lr = 3.2e-5
    chkpt =0
    for epoch in range(args.epochs):
        ...
        if val_loss < min_loss:
            ...
            # 保存模型语句
            torch.save(model.state_dict(),args.save_path+str(epoch+chkpt)+'.pth')
        # GPU memory usage
...
def search_best_pth(path,metric = 'mae'):
    ...
    path = [p for p in path if int(p[:-4])>40]
...
if __name__ == "__main__" : 
    ...
    if moshi == 'test':
        print('开始测试：')
        best_path = '198.pth'#for 15 #'91.pth' for 60
        _ = time.time()
...
if __name__ == "__main__" and moshi  == 'curve':
    ...
    configs1 = Configs1(args, n_vertex)
    loss, es, model, optimizer, scheduler = prepare_model(args, n_vertex)
    model.load_state_dict(torch.load(args.save_path+'338.pth'))
